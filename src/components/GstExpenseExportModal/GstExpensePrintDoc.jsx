...
const GstExpensePrintDoc = forwardRef((props, ref) => {
  const {
  ...
  const totalCredit = (records || []).reduce(
    (sum, item) => sum + (Number(item?.sended_amount) || 0),
    0,
  );
  ...
  const hasVendorFilter = Boolean(selectedVendor?.label || selectedVendor?.name);
  ...
  return (
    <div ref={ref} className="p-8 bg-white text-gray-900 font-sans print-document">
      ...
      <div className="bg-gray-50 border border-gray-200 rounded-md p-3 mb-4 text-xs grid grid-cols-1 sm:grid-cols-3 gap-2">
        <div>
          <span className="text-gray-500 font-medium block">Vendor Filter:</span>
          <span className="font-semibold text-gray-800">
            {hasVendorFilter ? vendorName : "All Vendors (No Filter)"}
          </span>
        </div>
        ...
        <table className="w-full border-collapse border border-gray-300 text-xs">
          ...
          <tbody>
            {records.map((item, index) => {
              ...
              const creditVal = Number(item.sended_amount) || 0;
...
GstExpensePrintDoc.displayName = "GstExpensePrintDoc";

export default GstExpensePrintDoc;
