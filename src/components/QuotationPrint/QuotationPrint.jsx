...
const QuotationPrint = forwardRef((props, ref) => {
  ...
  return (
    <div
      ref={ref}
      className="w-[210mm] min-h-[297mm] p-[10mm] bg-white text-gray-900 font-sans text-xs leading-normal mx-auto shadow-sm print:shadow-none print:m-0 print:p-0 print:w-full"
    ...
    >
      ...
      <div className="flex justify-between items-center border-b-2 border-[#2A3042] pb-3 mb-4">
        ...
        <div className="text-right space-y-1">
          <div className="flex items-center justify-end gap-1 text-[12px] font-medium text-gray-700">
            <PhoneIcon />
      ...
      <div className="grid grid-cols-2 gap-4 border border-gray-300 rounded-xs p-3 mb-4 bg-gray-50/50">
        {/* Customer Info */}
      ...
      {topText && (
        <div
          className="mb-4 text-xs leading-relaxed text-gray-800 whitespace-pre-wrap break-words"
      ...
      <div className="mb-4">
        <table className="w-full border-collapse border border-gray-400 text-xs">
          <thead>
            <tr className="bg-[#2A3042] text-white">
              <th className="border border-gray-400 py-1.5 px-2 text-center w-8 font-semibold">
                #
              </th>
              {columns.map((col) => (
                <th
                  ...
                  className={`border border-gray-400 py-1.5 px-2.5 font-semibold text-${col.align}`}
                >
                  {col.label}
                </th>
              ))}
          ...
          <tbody>
            {rows.map((row, rIdx) => (
              ...
              >
                <td className="border border-gray-300 py-1.5 px-2 text-center align-top text-gray-600 font-medium">
                  {rIdx + 1}
                </td>
                {columns.map((col) => (
                  <td
                    key={col.id}
                    className={`border border-gray-300 py-1.5 px-2.5 align-top text-${col.align} whitespace-pre-wrap`}
                  >
                    {col.key === "amount" ? (
        ...
        {/* Financial Summary */}
        <div className="flex justify-between items-start border-x border-b border-gray-400 p-2.5 bg-gray-50">
          ...
          <div className="w-[40%] space-y-1 text-xs">
            ...
            <div className="flex justify-between text-sm font-bold text-[#2A3042] border-t border-gray-300 pt-1.5">
      ...
      {bottomText && (
        <div className="my-4 p-3 border border-gray-200 rounded-xs bg-gray-50/40">
          <div
            className="text-xs leading-relaxed text-gray-800 whitespace-pre-wrap break-words"
